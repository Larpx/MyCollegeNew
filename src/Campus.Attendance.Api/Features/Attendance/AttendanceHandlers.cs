...
namespace Campus.Attendance.Api.Features.Attendance;
    ...
    IRequestHandler<MarkRandomPickCommand, ApiResponse<object>>
{
    ...
    public async Task<ApiResponse<SessionResponseDto>> Handle(CreateSessionCommand command, CancellationToken cancellationToken)
    {
        ...
        {
            CourseId = command.Dto.CourseId, ClassId = command.Dto.ClassId, TeacherId = command.TeacherId,
            ScheduleId = command.Dto.ScheduleId, StartTime = startTime, EndTime = endTime,
            Status = SessionStatus.Active, QrToken = null, CreateTime = DateTime.UtcNow
        };
    ...
    public async Task<ApiResponse<SessionResponseDto>> Handle(GetSessionByIdQuery query, CancellationToken cancellationToken)
    {
        ...
        var dto = await db.Queryable<AttendanceSession, Course, Class, Teacher>((s, c, cls, t) =>
            ...
            .Select((s, c, cls, t) => new SessionResponseDto
            {
                ...
                ScheduleId = s.ScheduleId, StartTime = s.StartTime, EndTime = s.EndTime,
                Status = s.Status, QrToken = s.QrToken, CreateTime = s.CreateTime
            }).FirstAsync();
    ...
    public async Task<ApiResponse<object>> Handle(CloseSessionCommand command, CancellationToken cancellationToken)
    {
        ...
        {
            SessionId = command.SessionId, StudentId = s.Id, StudentName = s.Name,
            Status = AttendanceStatus.Absent, CheckInTime = null,
            Remark = Msg.Attendance.AutoAbsentRemark, CreateTime = DateTime.UtcNow
        }).ToList();
    ...
    public async Task<ApiResponse<List<AttendanceRecordResponseDto>>> Handle(GetSessionRecordsQuery query, CancellationToken cancellationToken)
    {
        ...
        var rows = await db.Queryable<AttendanceRecord>()
            ...
            {
                Id = r.Id, SessionId = r.SessionId, StudentId = r.StudentId,
                StudentName = r.StudentName, Status = r.Status,
                CheckInTime = r.CheckInTime, Remark = r.Remark
            }).ToListAsync();
    ...
    public async Task<ApiResponse<QrCodeResult>> Handle(GenerateQrCodeCommand command, CancellationToken cancellationToken)
    {
        ...
        {
            Token = token, Base64Image = base64Image,
    ...
    public async Task<ApiResponse<CheckInResult>> Handle(CheckInCommand command, CancellationToken cancellationToken)
    {
        ...
        {
            SessionId = command.SessionId, StudentId = command.StudentId, StudentName = student.Name,
            Status = status, CheckInTime = checkInTime, Remark = null, CreateTime = DateTime.UtcNow
        };
    ...
    public async Task<ApiResponse<int>> Handle(RollCallCommand command, CancellationToken cancellationToken)
    {
        ...
        {
            SessionId = command.SessionId, StudentId = s.Id, StudentName = s.Name,
            Status = AttendanceStatus.Present, CheckInTime = checkInTime,
            Remark = Msg.Attendance.ManualCheckInRemark, CreateTime = DateTime.UtcNow
        }).ToList();
    ...
    public async Task<ApiResponse<AttendanceRecordResponseDto>> Handle(ManualCheckInCommand command, CancellationToken cancellationToken)
    {
        ...
        {
            SessionId = command.SessionId, StudentId = command.StudentId, StudentName = student.Name,
            Status = command.Status, CheckInTime = DateTime.UtcNow,
            Remark = Msg.Attendance.ManualCheckIn, CreateTime = DateTime.UtcNow
        };
    ...
    public async Task<ApiResponse<RandomPickResult>> Handle(RandomPickQuery query, CancellationToken cancellationToken)
    {
        ...
        {
            StudentId = picked.Id, StudentName = picked.Name,
            ClassId = query.ClassId, ClassName = cls.Name
        });
    }
    ...
    private (long SessionId, string? ErrorMessage) ValidateQrToken(string token, long expectedSessionId)
    {
        ...
        try
        {
            ...
            var principal = _tokenHandler.ValidateToken(token, new TokenValidationParameters
            {
                ...
                ValidIssuer = config.Issuer, ValidAudience = config.Audience, IssuerSigningKey = _signingKey,
    ...
    {
        Id = record.Id, SessionId = record.SessionId, StudentId = record.StudentId,
